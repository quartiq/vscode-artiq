...
let cacheGeometry = (item: GridItemHTMLElement, l: manager.Leaf) => {
    if (item.gridstackNode === undefined) return;
    let { x, y, w, h } = item.gridstackNode;
    l.geometry = { x, y, w, h };
};

let revealItem = (item: GridItemHTMLElement, l: manager.Leaf, grid: GridStack) => {
...
let syncVis = (item: GridItemHTMLElement, leaf: manager.Leaf, grid: GridStack) => {
...
export let newManagerItem = (grid: GridStack, onDelete: (leafs: manager.Leaf[]) => void) => {
    ...
    let handlers = {
        ...
        onVisibilityChanged: (leafs: manager.Leaf[]) => {
            let tuples = leafs
                ...
                .map((l): [ GridItemHTMLElement | undefined, manager.Leaf ] => [ findItem(keystr([ l.group, l.name ]), grid), l ])
...
export let newTemplateItem = async (args: KwargTypes["create_applet"], leaf: manager.Leaf, grid: GridStack): Promise<TemplateItem | undefined> => {
