...
let inputProps = {
    ...
    Value: { path: [1], desc: "PYON v2 JSON", test: (s) => pyonutils.validate(s, pyon.parse), parse: pyon.parse },
    Unit: { path: [2, "unit"], desc: "string", test: (s) => typeof s === "string", parse: (s) => s },
    Scale: { path: [2, "scale"], desc: "number", test: (s) => /^-?\d+(\.\d+)?$/.test(s), parse: (s) => Number(s) },
    // see: https://numpy.org/doc/stable/reference/generated/numpy.format_float_positional.html
    Precision: { path: [2, "precision"], desc: "non-negative integer", test: (s) => /^(0|[1-9][0-9]*)$/.test(s), parse: (s) => Number(s) },
};
let name = (keypath) => keypath.split(".").pop();
// makes no implicit statement regarding overflow one way or the other
let startsWith = (arr, prefix) => arr
    .slice(0, prefix.length)
    .every((val, index) => val === prefix[index]);
let findChildren = (keypaths, prefix, depth) => keypaths
    .map(kp => kp.split("."))
    .filter(keys => startsWith(keys, prefix))
    .filter(keys => keys.length >= prefix.length + (depth ?? 0));
let isNode = (keypath) => findChildren(Object.keys(sets.data), keypath.split(".")).length > 0;
let closestParent = (keypath) => {
    if (!keypath || Object.keys(sets.data).length === 0) {
        return undefined;
    }
...
let submit = async (setpath, set) => await net.rpc("dataset_db", "set", [setpath, set?.[1], set?.[0], set?.[2]]);
let applyScale = (value, meta, inverse) => {
...
let applyPrecision = (value, precision) => {
...
export let del = async (keypath) => {
    let result = await vscode.window.showWarningMessage(`Do you really want to delete dataset "${keypath}"?`, { modal: true }, "Delete");
    if (result === "Delete") {
        net.rpc("dataset_db", "delete", [keypath]);
    }
};
// see: m-labs/artiq/dashboard/datasets:CreateEditDialog.accept
export let edit = async (keypath, propname) => {
