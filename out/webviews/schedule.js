    ...
    for (var i in obj) {
      ...
      if (subject != null && typeof subject === "object" && (subject.__proto__ === objectProto || subject.__proto__ === arrayProto)) {
        ...
        if (match > -1) {
          ...
          list2.unshift({ subject, copy: copy10 });
          clone[i] = this.deepClone(subject, copy10, list2);
        }
      }
    }
    return clone;
...
var DataTree = class extends Module {
  ...
  columnMoving() {
    ...
    this.table.rowManager.rows.forEach((row) => {
      rows3 = rows3.concat(this.getTreeChildren(row, false, true));
    });
    return rows3;
  }
...
function xlsx$1(list2, options, setFileContents) {
  ...
  function generateSheet() {
    ...
    XLSXLib.utils.sheet_add_aoa(worksheet, rows3);
...
var Export = class _Export extends Module {
  ...
  rowLookup(range2) {
    ...
    if (typeof range2 == "function") {
      ...
      rows3 = rowLookup.call(this.table);
    }
  ...
  bodyToExportRows(rows3, columns = []) {
    var exportRows = [];
    if (columns.length === 0) {
      this.table.columnManager.columnsByIndex.forEach((column) => {
        if (this.columnVisCheck(column)) {
  ...
  mapElementStyles(from3, to, props) {
    if (this.cloneTableStyle && from3 && to) {
      var lookup2 = {
      ...
      if (window.getComputedStyle) {
        var fromStyle = window.getComputedStyle(from3);
...
var GroupRows = class extends Module {
  ...
  ///////// Internal Logic //////////
  ///////////////////////////////////
  rowMoving(from3, to, after) {
    if (this.table.options.groupBy) {
      if (!after && to instanceof Group) {
        to = this.table.rowManager.prevDisplayRow(from3) || to;
      }
      var toGroup = to instanceof Group ? to : to.modules.group;
      var fromGroup = from3 instanceof Group ? from3 : from3.modules.group;
      if (toGroup === fromGroup) {
        this.table.rowManager.moveRowInArray(toGroup.rows, from3, to, after);
      } else {
        if (fromGroup) {
          fromGroup.removeRow(from3);
        }
        toGroup.insertRow(from3, to, after);
      }
    }
  }
  rowDeleting(row) {
...
var History = class _History extends Module {
  ...
  rowMoved(from3, to, after) {
    this.action("rowMove", from3, { posFrom: from3.getPosition(), posTo: to.getPosition(), to, after });
  }
  ...
  rowDeleted(row) {
    var index, rows3;
    if (this.table.options.groupBy) {
      rows3 = row.getComponent().getGroup()._getSelf().rows;
      index = rows3.indexOf(row);
      if (index) {
        index = rows3[index - 1];
      }
    } else {
...
var HtmlTableImport = class extends Module {
  ...
  parseTable() {
    ...
    for (var index = 0; index < rows3.length; index++) {
      var row = rows3[index], cells = row.getElementsByTagName("td"), item = {};
      if (!this.hasIndex) {
...
var Interaction = class extends Module {
  ...
  clearTouchWatchers() {
    var types2 = Object.values(this.touchWatchers);
    types2.forEach((type) => {
      for (let key in type) {
        type[key] = null;
      }
...
var Persistence = class _Persistence extends Module {
  ...
  mergeDefinition(oldCols, newCols, mergeAllNew) {
    ...
    newCols.forEach((column, to) => {
      var from3 = this._findColumn(oldCols, column), keys;
      if (from3) {
        if (mergeAllNew) {
          keys = Object.keys(column);
        } else if (this.config.columns === true || this.config.columns == void 0) {
          keys = Object.keys(from3);
          keys.push("width");
        } else {
        ...
        keys.forEach((key) => {
          if (key !== "columns" && typeof column[key] !== "undefined") {
            from3[key] = column[key];
          }
        });
        if (from3.columns) {
          from3.columns = this.mergeDefinition(from3.columns, column.columns);
        }
        output.push(from3);
      }
    });
    oldCols.forEach((column, i) => {
      var from3 = this._findColumn(newCols, column);
      if (!from3) {
        if (output.length > i) {
...
var Range = class extends CoreFeature {
  ...
  getData() {
    var data = [], rows3 = this.getRows(), columns = this.getColumns();
    rows3.forEach((row) => {
  ...
  getCells(structured, component) {
    var cells = [], rows3 = this.getRows(), columns = this.getColumns();
    if (structured) {
      cells = rows3.map((row) => {
        var arr = [];
        row.getCells().forEach((cell) => {
...
var pasteActions = {
  range: function(data) {
    var rows3 = [], range2 = this.table.modules.selectRange.activeRange, singleCell = false, bounds, startCell, startRow, rowWidth, dataLength;
    ...
    if (range2) {
      ...
      if (startCell) {
        rows3 = this.table.rowManager.activeRows.slice();
        startRow = rows3.indexOf(startCell.row);
        if (singleCell) {
          rowWidth = data.length;
        } else {
          rowWidth = rows3.indexOf(bounds.end.row) - startRow + 1;
        }
        if (startRow > -1) {
          this.table.blockRedraw();
          rows3 = rows3.slice(startRow, startRow + rowWidth);
          rows3.forEach((row, i) => {
...
var pasteParsers = {
  range: function(clipboard) {
    var data = [], rows3 = [], range2 = this.table.modules.selectRange.activeRange, singleCell = false, bounds, startCell, colWidth, columnMap, startCol;
    if (range2) {
      bounds = range2.getBounds();
      startCell = bounds.start;
      if (bounds.start === bounds.end) {
...
var SelectRange = class extends Module {
  ...
  handleColumnMoved(from3, _to, _after) {
    this.activeRange.setBounds(from3);
    this.layoutElement();
  }
  ...
  findJumpRow(column, rows3, reverse, emptyStart, emptySide) {
    if (reverse) {
      rows3 = rows3.reverse();
    }
    return this.findJumpItem(emptyStart, emptySide, rows3, function(row) {
  ...
  findJumpCellUp(rowPos, colPos) {
    var column = this.getColumnByRangePos(colPos), rows3 = this.getTableRows(), isStartingCellEmpty = this.isEmpty(rows3[rowPos].getData()[column.getField()]), isTopOfStartingCellEmpty = rows3[rowPos - 1] ? this.isEmpty(rows3[rowPos - 1].getData()[column.getField()]) : false, jumpRow = this.findJumpRow(column, rows3.slice(0, rowPos), true, isStartingCellEmpty, isTopOfStartingCellEmpty);
    if (jumpRow) {
  ...
  findJumpCellDown(rowPos, colPos) {
    var column = this.getColumnByRangePos(colPos), rows3 = this.getTableRows(), isStartingCellEmpty = this.isEmpty(rows3[rowPos].getData()[column.getField()]), isBottomOfStartingCellEmpty = rows3[rowPos + 1] ? this.isEmpty(rows3[rowPos + 1].getData()[column.getField()]) : false, jumpRow = this.findJumpRow(column, rows3.slice(rowPos + 1, rows3.length), false, isStartingCellEmpty, isBottomOfStartingCellEmpty);
    if (jumpRow) {
      return jumpRow.position - 1;
    }
    return rowPos;
  ...
  layoutElement(visibleRows) {
    var rows3;
    if (visibleRows) {
      rows3 = this.table.rowManager.getVisibleRows(true);
    } else {
      rows3 = this.table.rowManager.getRows();
    }
    rows3.forEach((row) => {
...
var SheetComponent = class {
  ...
  setRows(rows3) {
    return this._sheet.setRows(rows3);
  }
  setColumns(columns) {
...
var ColumnManager = class extends CoreFeature {
  ...
  _moveColumnInArray(columns, from3, to, after, updateRows) {
    var fromIndex = columns.indexOf(from3), toIndex, rows3 = [];
    if (fromIndex > -1) {
      ...
      if (updateRows) {
        rows3 = this.chain("column-moving-rows", [from3, to, after], null, []) || [];
...
var VirtualDomVertical = class extends Renderer {
  ...
  rerenderRows(callback) {
    ...
    for (var i = this.vDomTop; i <= this.vDomBottom; i++) {
      if (rows3[i]) {
        var diff = scrollTop - rows3[i].getElement().offsetTop;
  ...
  visibleRows(includingBuffer) {
    ...
    if (includingBuffer) {
      ...
      for (var i = this.vDomTop; i <= this.vDomBottom; i++) {
        if (rows3[i]) {
          if (!topFound) {
            if (topEdge - rows3[i].getElement().offsetTop >= 0) {
              topRow = i;
            } else {
              topFound = true;
              if (bottomEdge - rows3[i].getElement().offsetTop >= 0) {
                bottomRow = i;
              } else {
                break;
              }
    ...
    if (rowsCount && Helpers.elVisible(this.elementVertical)) {
      ...
      while ((rowsToRender == rowsCount || rowsHeight <= containerHeight + this.vDomWindowBuffer || totalRowsRendered < this.vDomWindowMinTotalRows) && this.vDomBottom < rowsCount - 1) {
        ...
        while (i < rowsToRender && this.vDomBottom < rowsCount - 1) {
          index = this.vDomBottom + 1, row = rows3[index];
          this.styleRow(row, index);
          row.initialize(false, true);
  ...
  _addTopRow(rows3, fillableSpace) {
    ...
    while (working) {
      if (this.vDomTop) {
        let row = rows3[index], rowHeight, initialized;
  ...
  _removeTopRow(rows3, fillableSpace) {
    ...
    while (working) {
      let row = rows3[this.vDomTop], rowHeight;
      if (row && i < this.vDomMaxRenderChain) {
        rowHeight = row.getHeight() || this.vDomRowHeight;
        if (fillableSpace >= rowHeight) {
          this.vDomTop++;
  ...
  _addBottomRow(rows3, fillableSpace) {
    ...
    if (paddingAdjust) {
      ...
      if (this.vDomBottomPad < 0 || index == rows3.length - 1) {
        this.vDomBottomPad = 0;
      }
  ...
  _removeBottomRow(rows3, fillableSpace) {
    var removableRows = [], paddingAdjust = 0, i = 0, working = true;
    while (working) {
      let row = rows3[this.vDomBottom], rowHeight;
      if (row && i < this.vDomMaxRenderChain) {
        rowHeight = row.getHeight() || this.vDomRowHeight;
        if (fillableSpace >= rowHeight) {
          this.vDomBottom--;
...
var RowManager = class extends CoreFeature {
  ...
  deleteRow(row, blockRedraw) {
    ...
    this.displayRowIterator((rows3) => {
      var displayIndex = rows3.indexOf(row);
      if (displayIndex > -1) {
  ...
  addRowActual(data, pos, index, blockRedraw) {
    ...
    if (index && allIndex > -1) {
      ...
      this.displayRowIterator(function(rows3) {
        var displayIndex = rows3.indexOf(index);
        if (displayIndex > -1) {
          rows3.splice(top ? displayIndex : displayIndex + 1, 0, row);
        }
      });
      if (activeIndex > -1) {
  ...
  moveRowActual(from3, to, after) {
    ...
    this.displayRowIterator((rows3) => {
      this.moveRowInArray(rows3, from3, to, after);
    });
    this.dispatch("row-moving", from3, to, after);
  }
  moveRowInArray(rows3, from3, to, after) {
    var fromIndex, toIndex, start, end;
    if (from3 !== to) {
      fromIndex = rows3.indexOf(from3);
      if (fromIndex > -1) {
        rows3.splice(fromIndex, 1);
        toIndex = rows3.indexOf(to);
        if (toIndex > -1) {
          if (after) {
            rows3.splice(toIndex + 1, 0, from3);
          } else {
            rows3.splice(toIndex, 0, from3);
          }
        } else {
          rows3.splice(fromIndex, 0, from3);
        }
      }
...
var Tabulator = class _Tabulator extends ModuleBinder {
  ...
  addData(data, pos, index) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (data) {
        this.rowManager.addRows(data, pos, index).then((rows3) => {
          var output = [];
          rows3.forEach(function(row) {
            output.push(row.getComponent());
          });
  ...
  addRow(data, pos, index) {
    ...
    return this.rowManager.addRows(data, pos, index, true).then((rows3) => {
...
var table = new TabulatorFull$1(".table", {
  layout: "fitDataFill",
  columns: [
    { title: "RID", field: "rid" },
  ...
  rowContextMenu: [
    ...
    },
    {
      ...
      action: (ev, row) => rows2.filter((r) => r.pipeline === row.getData().pipeline).forEach((r) => rpc("request_termination", r.rid))
    }
  ]
});
window.addEventListener("message", (ev) => {
  rows2 = Object.entries(decode(ev.data)).map(([rid, syncinfo]) => ({ rid, ...syncinfo }));
  table.replaceData(rows2);
});
