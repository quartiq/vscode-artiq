...
var Accessor = class _Accessor extends Module {
  ...
  initializeColumn(column) {
    ...
    this.allowedTypes.forEach((type) => {
      var key = "accessor" + (type.charAt(0).toUpperCase() + type.slice(1)), accessor2;
      if (column.definition[key]) {
        accessor2 = this.lookupAccessor(column.definition[key]);
        if (accessor2) {
          match = true;
          config[key] = {
            accessor: accessor2,
    ...
    switch (typeof value) {
      case "string":
        if (_Accessor.accessors[value]) {
          accessor2 = _Accessor.accessors[value];
        } else {
  ...
  transformRow(row, type) {
    ...
    this.table.columnManager.traverse(function(column) {
      var value, accessor2, params, colComponent;
      if (column.modules.accessor) {
        accessor2 = column.modules.accessor[key] || column.modules.accessor.accessor || false;
        if (accessor2) {
          value = column.getFieldValue(data);
          if (value != "undefined") {
            colComponent = column.getComponent();
            params = typeof accessor2.params === "function" ? accessor2.params(value, data, type, colComponent, rowComponent) : accessor2.params;
            column.setFieldValue(data, accessor2.accessor(value, data, type, params, colComponent, rowComponent));
          }
        }
      }
    });
      ...
      this.table.element.addEventListener("copy", (e) => {
        ...
        if (!this.blocked) {
          ...
          if (this.customSelection) {
            ...
            html3 = this.table.modules.export.generateHTMLTable(list2);
            plain = html3 ? this.generatePlainContent(list2) : "";
            if (this.table.options.clipboardCopyFormatter) {
              plain = this.table.options.clipboardCopyFormatter("plain", plain);
              html3 = this.table.options.clipboardCopyFormatter("html", html3);
            }
          }
          if (window.clipboardData && window.clipboardData.setData) {
            ...
            if (html3) {
              e.originalEvent.clipboardData.setData("text/html", html3);
            }
          }
          this.dispatchExternal("clipboardCopied", plain, html3);
          this.reset();
        }
      });
    }
...
function input(cell, onRendered, success, cancel, editorParams) {
  var cellValue = cell.getValue(), input3 = document.createElement("input");
  input3.setAttribute("type", editorParams.search ? "search" : "text");
  input3.style.padding = "4px";
  ...
  if (editorParams.elementAttributes && typeof editorParams.elementAttributes == "object") {
    for (let key in editorParams.elementAttributes) {
      if (key.charAt(0) == "+") {
        key = key.slice(1);
        input3.setAttribute(key, input3.getAttribute(key) + editorParams.elementAttributes["+" + key]);
      } else {
        input3.setAttribute(key, editorParams.elementAttributes[key]);
      }
    }
  }
  input3.value = typeof cellValue !== "undefined" ? cellValue : "";
  onRendered(function() {
    if (cell.getType() === "cell") {
      input3.focus({ preventScroll: true });
  ...
  function onChange(e) {
    if ((cellValue === null || typeof cellValue === "undefined") && input3.value !== "" || input3.value !== cellValue) {
      if (success(input3.value)) {
        cellValue = input3.value;
      }
    } else {
      cancel();
    }
  ...
  input3.addEventListener("blur", onChange);
  ...
  if (editorParams.mask) {
    maskInput(input3, editorParams);
  }
  return input3;
}
function textarea$1(cell, onRendered, success, cancel, editorParams) {
  var cellValue = cell.getValue(), vertNav = editorParams.verticalNavigation || "hybrid", value = String(cellValue !== null && typeof cellValue !== "undefined" ? cellValue : ""), input3 = document.createElement("textarea"), scrollHeight = 0;
  input3.style.display = "block";
  input3.style.padding = "2px";
  input3.style.height = "100%";
  input3.style.width = "100%";
  input3.style.boxSizing = "border-box";
  input3.style.whiteSpace = "pre-wrap";
  ...
  onRendered(function() {
    if (cell.getType() === "cell") {
      ...
      input3.style.height = "100%";
      input3.scrollHeight;
      input3.style.height = input3.scrollHeight + "px";
  ...
  input3.addEventListener("keyup", function() {
    input3.style.height = "";
    var heightNow = input3.scrollHeight;
    input3.style.height = heightNow + "px";
  ...
  input3.addEventListener("keydown", function(e) {
    switch (e.keyCode) {
      ...
      case 27:
        ...
        break;
      case 38:
        if (vertNav == "editor" || vertNav == "hybrid" && input3.selectionStart) {
          e.stopImmediatePropagation();
          e.stopPropagation();
        }
        break;
      case 40:
        if (vertNav == "editor" || vertNav == "hybrid" && input3.selectionStart !== input3.value.length) {
          e.stopImmediatePropagation();
          e.stopPropagation();
        }
        break;
...
function number$1(cell, onRendered, success, cancel, editorParams) {
  var cellValue = cell.getValue(), vertNav = editorParams.verticalNavigation || "editor", input3 = document.createElement("input");
  input3.setAttribute("type", "number");
  if (typeof editorParams.max != "undefined") {
    input3.setAttribute("max", editorParams.max);
  }
  if (typeof editorParams.min != "undefined") {
    input3.setAttribute("min", editorParams.min);
  }
  if (typeof editorParams.step != "undefined") {
    input3.setAttribute("step", editorParams.step);
  }
  input3.style.padding = "4px";
  input3.style.width = "100%";
  input3.style.boxSizing = "border-box";
  ...
  input3.value = cellValue;
  var blurFunc = function(e) {
    onChange();
  };
  onRendered(function() {
    if (cell.getType() === "cell") {
      input3.removeEventListener("blur", blurFunc);
      input3.focus({ preventScroll: true });
      input3.style.height = "100%";
...
function date$1(cell, onRendered, success, cancel, editorParams) {
  ...
  input3.style.boxSizing = "border-box";
  if (editorParams.max) {
    input3.setAttribute("max", inputFormat ? convertDate(editorParams.max) : editorParams.max);
  }
  if (editorParams.min) {
    input3.setAttribute("min", inputFormat ? convertDate(editorParams.min) : editorParams.min);
  }
  if (editorParams.elementAttributes && typeof editorParams.elementAttributes == "object") {
    for (let key in editorParams.elementAttributes) {
      if (key.charAt(0) == "+") {
  ...
  function onChange() {
    var value = input3.value, luxDate;
  ...
  input3.addEventListener("blur", function(e) {
    if (e.relatedTarget || e.rangeParent || e.explicitOriginalTarget !== input3) {
      onChange();
    }
  });
  input3.addEventListener("keydown", function(e) {
...
function time$1(cell, onRendered, success, cancel, editorParams) {
  ...
  function onChange() {
    var value = input3.value, luxTime;
    if ((cellValue === null || typeof cellValue === "undefined") && value !== "" || value !== cellValue) {
      if (value && inputFormat) {
...
function datetime$2(cell, onRendered, success, cancel, editorParams) {
  ...
  function onChange() {
    var value = input3.value, luxDateTime;
    if ((cellValue === null || typeof cellValue === "undefined") && value !== "" || value !== cellValue) {
      if (value && inputFormat) {
...
var Edit$1 = class Edit {
  constructor(editor2, cell, onRendered, success, cancel, editorParams) {
    this.edit = editor2;
    this.table = editor2.table;
    this.cell = cell;
  ...
  _createInputElement() {
    ...
    input3.setAttribute("type", this.params.clearable ? "search" : "text");
    input3.style.padding = "4px";
    input3.style.width = "100%";
    input3.style.boxSizing = "border-box";
    if (!this.params.autocomplete) {
      input3.style.cursor = "default";
      input3.style.caretColor = "transparent";
    }
    if (attribs && typeof attribs == "object") {
      for (let key in attribs) {
        if (key.charAt(0) == "+") {
          key = key.slice(1);
          input3.setAttribute(key, input3.getAttribute(key) + attribs["+" + key]);
        } else {
          input3.setAttribute(key, attribs[key]);
        }
      }
    }
    if (this.params.mask) {
      maskInput(input3, this.params);
    }
    this._bindInputEvents(input3);
    return input3;
  }
  ...
  ////////// Event Handling ////////////
  //////////////////////////////////////
  _bindInputEvents(input3) {
    input3.addEventListener("focus", this._inputFocus.bind(this));
    input3.addEventListener("click", this._inputClick.bind(this));
    input3.addEventListener("blur", this._inputBlur.bind(this));
    input3.addEventListener("keydown", this._inputKeyDown.bind(this));
    input3.addEventListener("search", this._inputSearch.bind(this));
    if (this.params.autocomplete) {
      input3.addEventListener("keyup", this._inputKeyUp.bind(this));
    }
  }
  _inputFocus(e) {
...
function tickCross$1(cell, onRendered, success, cancel, editorParams) {
  ...
  input3.checked = trueValueSet ? value === editorParams.trueValue : value === true || value === "true" || value === "True" || value === 1;
  function setValue(blur) {
    var checkedValue = input3.checked;
    ...
    if (tristate) {
      if (!blur) {
        if (input3.checked && !indetermState) {
...
function adaptable$1(cell, onRendered, success, cancel, params) {
  ...
  function defaultLookup(cell2) {
    var value = cell2.getValue(), editor2 = "input";
    switch (typeof value) {
      case "number":
        editor2 = "number";
        break;
...
var Edit2 = class _Edit extends Module {
  ...
  lookupEditor(editor2, column) {
    var editorFunc;
    switch (typeof editor2) {
      case "string":
        if (this.editors[editor2]) {
          editorFunc = this.editors[editor2];
        } else {
          console.warn("Editor Error - No such editor found: ", editor2);
        }
        break;
      case "function":
        editorFunc = editor2;
...
var Export = class _Export extends Module {
  ...
  mapElementStyles(from2, to, props) {
    if (this.cloneTableStyle && from2 && to) {
      var lookup2 = {
...
var Filter = class _Filter extends Module {
  ...
  generateHeaderFilterElement(column, initialValue, reinitialize) {
    var self = this, success = column.modules.filter.success, field = column.getField(), filterElement, editor2, editorElement, cellWrapper, typingTimer, searchTrigger, params, onRenderedCallback;
    ...
    if (field) {
      ...
      switch (typeof column.definition.headerFilter) {
        case "string":
          if (self.table.modules.edit.editors[column.definition.headerFilter]) {
            editor2 = self.table.modules.edit.editors[column.definition.headerFilter];
          ...
          break;
        case "function":
          editor2 = column.definition.headerFilter;
          break;
        case "boolean":
          if (column.modules.edit && column.modules.edit.editor) {
            editor2 = column.modules.edit.editor;
          } else {
            if (column.definition.formatter && self.table.modules.edit.editors[column.definition.formatter]) {
              editor2 = self.table.modules.edit.editors[column.definition.formatter];
              ...
              editor2 = self.table.modules.edit.editors["input"];
            }
          }
          break;
      }
      if (editor2) {
        ...
        editorElement = editor2.call(this.table.modules.edit, cellWrapper, onRendered, success, cancel, params);
        if (!editorElement) {
...
function adaptable(cell, params, onRendered) {
  ...
  function defaultLookup(cell2) {
    var value = cell2.getValue(), formatter2 = "plaintext";
    switch (typeof value) {
      case "boolean":
        formatter2 = "tickCross";
        break;
      case "string":
        if (value.includes("\n")) {
          formatter2 = "textarea";
...
var Format = class _Format extends Module {
  ...
  lookupTypeFormatter(column, type) {
    var config = { params: column.definition["formatter" + type + "Params"] || {} }, formatter2 = column.definition["formatter" + type];
    config.formatter = this.lookupFormatter(formatter2);
    return config;
  }
  lookupFormatter(formatter2) {
    var formatterFunc;
    switch (typeof formatter2) {
      case "string":
        if (_Format.formatters[formatter2]) {
          formatterFunc = _Format.formatters[formatter2];
        } else {
          console.warn("Formatter Error - No such formatter found: ", formatter2);
          formatterFunc = _Format.formatters.plaintext;
        }
        break;
      case "function":
        formatterFunc = formatter2;
        break;
      default:
        formatterFunc = _Format.formatters.plaintext;
        break;
  ...
  formatHeader(column, title, el) {
    var formatter2, params, onRendered, mockCell;
    if (column.definition.titleFormatter) {
      formatter2 = this.lookupFormatter(column.definition.titleFormatter);
      onRendered = (callback) => {
      ...
      return formatter2.call(this, mockCell, params, onRendered);
    } else {
      return title;
    }
  }
  ...
  formatExportValue(cell, type) {
    var formatter2 = cell.column.modules.format[type], params;
    if (formatter2) {
      ...
      params = typeof formatter2.params === "function" ? formatter2.params(cell.getComponent()) : formatter2.params;
      return formatter2.formatter.call(this, cell.getComponent(), params, onRendered);
    } else {
...
var GroupRows = class extends Module {
  ...
  ///////// Internal Logic //////////
  ///////////////////////////////////
  rowMoving(from2, to, after) {
    if (this.table.options.groupBy) {
      if (!after && to instanceof Group) {
        to = this.table.rowManager.prevDisplayRow(from2) || to;
      }
      var toGroup = to instanceof Group ? to : to.modules.group;
      var fromGroup = from2 instanceof Group ? from2 : from2.modules.group;
      if (toGroup === fromGroup) {
        this.table.rowManager.moveRowInArray(toGroup.rows, from2, to, after);
      } else {
        if (fromGroup) {
          fromGroup.removeRow(from2);
        }
        toGroup.insertRow(from2, to, after);
      }
    }
  }
  rowDeleting(row) {
...
var History = class _History extends Module {
  ...
  rowMoved(from2, to, after) {
    this.action("rowMove", from2, { posFrom: from2.getPosition(), posTo: to.getPosition(), to, after });
  }
...
function csv(input3) {
  ...
  for (let index = 0; index < input3.length; index++) {
    let char = input3[index], nextChar = input3[index + 1];
    if (!data[row]) {
      data[row] = [];
    }
    if (!data[row][col]) {
...
function json(input3) {
  try {
    return JSON.parse(input3);
  } catch (e) {
    console.warn("JSON Import Error - File contents is invalid JSON", e);
    return Promise.reject();
  }
}
function array$1(input3) {
  return input3;
}
function xlsx(input3) {
  var XLSXLib = this.dependencyRegistry.lookup("XLSX"), workbook2 = XLSXLib.read(input3), sheet = workbook2.Sheets[workbook2.SheetNames[0]];
...
var Import = class _Import extends Module {
  ...
  pickFile(extensions2, importReader) {
    return new Promise((resolve, reject) => {
      var input3 = document.createElement("input");
      input3.type = "file";
      input3.accept = extensions2;
      input3.addEventListener("change", (e) => {
        var file = input3.files[0], reader = new FileReader(), valid = this.validateFile(file);
        if (valid === true) {
          this.dispatch("import-importing", input3.files);
          this.dispatchExternal("importImporting", input3.files);
...
var Mutator = class _Mutator extends Module {
  ...
  initializeColumn(column) {
    var match = false, config = {};
    this.allowedTypes.forEach((type) => {
      var key = "mutator" + (type.charAt(0).toUpperCase() + type.slice(1)), mutator2;
      if (column.definition[key]) {
        mutator2 = this.lookupMutator(column.definition[key]);
        if (mutator2) {
          match = true;
          config[key] = {
            mutator: mutator2,
    ...
    switch (typeof value) {
      case "string":
        if (_Mutator.mutators[value]) {
          mutator2 = _Mutator.mutators[value];
        } else {
  ...
  transformRow(data, type, updatedData) {
    ...
    if (this.enabled) {
      this.table.columnManager.traverse((column) => {
        var mutator2, params, component;
        if (column.modules.mutate) {
          mutator2 = column.modules.mutate[key] || column.modules.mutate.mutator || false;
          if (mutator2) {
            value = column.getFieldValue(typeof updatedData !== "undefined" ? updatedData : data);
            if (type == "data" && !updatedData || typeof value !== "undefined") {
              component = column.getComponent();
              params = typeof mutator2.params === "function" ? mutator2.params(value, data, type, component) : mutator2.params;
              column.setFieldValue(data, mutator2.mutator(value, data, type, params, component));
            }
          }
        }
      });
  ...
  transformCell(cell, value) {
    if (cell.column.modules.mutate) {
      var mutator2 = cell.column.modules.mutate.mutatorEdit || cell.column.modules.mutate.mutator || false, tempData = {};
      if (mutator2) {
        tempData = Object.assign(tempData, cell.row.getData());
        cell.column.setFieldValue(tempData, value);
        return mutator2.mutator(value, tempData, "edit", mutator2.params, cell.getComponent());
      }
    }
    return value;
  }
  ...
  initialize() {
    if (this.table.options.autoResize) {
      ...
      if (typeof IntersectionObserver !== "undefined" && typeof ResizeObserver !== "undefined" && table2.rowManager.getRenderMode() === "virtual") {
        ...
        this.resizeObserver = new ResizeObserver((entry2) => {
          if (!table2.browserMobile || table2.browserMobile && (!table2.modules.edit || table2.modules.edit && !table2.modules.edit.currentCell)) {
            var nodeHeight = Math.floor(entry2[0].contentRect.height);
            var nodeWidth = Math.floor(entry2[0].contentRect.width);
        ...
        if (this.table.element.parentNode && !this.table.rowManager.fixedHeight && (tableStyle.getPropertyValue("max-height") || tableStyle.getPropertyValue("min-height"))) {
          this.containerObserver = new ResizeObserver((entry2) => {
            if (!table2.browserMobile || table2.browserMobile && (!table2.modules.edit || table2.modules.edit && !table2.modules.edit.currentCell)) {
              var nodeHeight = Math.floor(entry2[0].contentRect.height);
              var nodeWidth = Math.floor(entry2[0].contentRect.width);
...
var SelectRange = class extends Module {
  ...
  handleColumnMoved(from2, _to, _after) {
    this.activeRange.setBounds(from2);
    this.layoutElement();
  }
...
var ColumnManager = class extends CoreFeature {
  ...
  _moveColumnInArray(columns, from2, to, after, updateRows) {
    ...
    if (fromIndex > -1) {
      ...
      if (updateRows) {
        rows2 = this.chain("column-moving-rows", [from2, to, after], null, []) || [];
...
var RowManager = class extends CoreFeature {
  ...
  moveRowActual(from2, to, after) {
    ...
    this.displayRowIterator((rows2) => {
      this.moveRowInArray(rows2, from2, to, after);
    });
    this.dispatch("row-moving", from2, to, after);
  }
  moveRowInArray(rows2, from2, to, after) {
...
var unixsecs = (date2) => Math.floor(date2.getTime() / 1e3);
var nowsecs = () => Math.floor(Date.now() / 1e3);
var datetimelocal = (secs) => {
  let date2 = new Date(secs * 1e3);
  let a = date2.getFullYear();
  let mo = String(date2.getMonth() + 1).padStart(2, "0");
  let d = String(date2.getDate()).padStart(2, "0");
  let h = String(date2.getHours()).padStart(2, "0");
  let min = String(date2.getMinutes()).padStart(2, "0");
...
var parseAll = (el) => {
  ...
  el.querySelectorAll("input").forEach((input3) => {
    let [ty, prop] = input3.id.split("__");
    (state[ty] ??= {})[prop] = parse(input3);
  });
  return state;
};
...
var initWidget = (el, info) => {
  ...
  ["Start", "Stop", "Npoints"].forEach((bound) => {
    let input3 = el.querySelector(`#RangeScan__${bound.toLowerCase()}`);
    widget[`on${bound}Changed`] = (v) => populate(input3, v, info.arg[0].precision);
    input3.addEventListener("change", () => widget[`set${bound}`](parse(input3)));
  });
  return widget;
};
var from = (info) => {
...
var NumberEntry = {
  ...
  editor: (info) => {
    ...
    let el = input2({ ...info, parse: parse2 });
...
var DatetimeEntry = {
  ...
  editor: (info) => {
    ...
    el.value = datetimelocal(info.arg[3]);
    return el;
  },
...
// src/webviews/experiment.ts
var vscode = acquireVsCodeApi();
var table;
var idleel = document.querySelector(".idle");
var msgels = idleel.querySelectorAll(".msg");
...
var accessor = (arg) => arg[3];
var formatter = (cell) => {
  let row = cell.getRow().getData();
  return entry(row.arg[0].ty)?.formatter(row.arg[3]);
};
...
var mutator = (value, data) => data.name === "Due date" && value === null ? nowsecs() : value;
var updateTable = (exp) => {
  let data = Object.entries(fields).map(([name, field]) => ({ name, arg: [field.procdesc, "", "", exp[field.key]] }));
  table?.destroy?.();
  table = new TabulatorFull$1(".table", {
    headerVisible: false,
    layout: "fitDataFill",
    data,
    columns: [
...
window.addEventListener("message", (ev) => actions2[ev.data.action](ev.data.data));
